RSpec.describe PayPal::SDK::Subscriptions::RequestBase do
  let(:error_object) {
    {
    ...
    context 'when there is error' do
      ...
      it 'raises error on request with all API information' do
        expect { subject.raise_error! }
          .to raise_error { |err|
            expect(err).to be_a(PayPal::SDK::Subscriptions::UnsuccessfulApiCall)
  ...
  context '.raise_on_api_error' do
    ...
    context 'when call is unsuccessful' do
      ...
      it { expect { subject.some_call! }.to raise_error(PayPal::SDK::Subscriptions::UnsuccessfulApiCall) }
    end
  end
end
